...
export const OptionsDashboard = ({ trades = [] }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      const optionTrades = trades.filter(t => t.market === 'b3options');
      ...
      const taxInfo = await calculatePeriodTax(optionTrades, 'b3options', currentPeriod, user?.uid);

      const winning = optionTrades.filter(t => t.pnl > 0);
      const losing = optionTrades.filter(t => t.pnl < 0);
      
      const pnls = optionTrades.map(t => parseFloat(t.pnl || 0));
      ...
      setStats({
        ...
        avgPnl: (taxInfo.consolidatedPnL || 0) / optionTrades.length,
        winRate: (winning.length / optionTrades.length) * 100,
        bestTrade,
        worstTrade,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">Opções B3</h1>
          <p className="text-zinc-400">Operações com opções na B3</p>
        </div>
      </div>
      ...
      <Card className="bg-amber-900/20 border-amber-500/50">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-zinc-300 text-sm">
              Opções têm alíquota de <strong>15%</strong> sobre o lucro consolidado mensal.
              <br />
