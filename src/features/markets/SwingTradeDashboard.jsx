...
export const SwingTradeDashboard = ({ trades = [] }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      const taxInfo = await calculatePeriodTax(swingTrades, 'b3swing', currentPeriod, user?.uid);
      ...
      setStats({
        ...
        avgPnl: (taxInfo.consolidatedPnL || 0) / swingTrades.length,
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-zinc-400">Operações de posição na B3</p>
        </div>
      </div>
